...
from general.general import read_file, get_location_input_files, measure, get_location
...
def walk_grid(grid, start_pos):
    ...
    while True:
        ...
        # done when there is no where to go anymore
        if turn == '':
            break
...
def main(args=None):
    ...
    img.save(get_location('output') / "grid_day17.png")
    save_grid(get_location('output') / 'grid_day17.txt', ascii_output)
